...
// 函数声明（此处声明包含了开发后期对前期做的不合要求的函数的适配器）
void Input(STU *p, int n);//适配Input2（）
int Max(STU *p, int scoreIndex);//适配MaxAll（）
void Sort_insert(STU *p, int n, char *major);//适配Sort_insert2（）
 int Input2(STU *p, const char *filename);//被Input封装
void Output(STU *p);
STU Fetch(int studentIndex);
void MaxAll(STU *p, int courseIndex);//被Max封装
...
void Sort_insert2(STU *p, int n, char *major, int courseIndex);//被Sort_insert封装
...
int main() {
    ...
    //从文件 currentFileName 中读取若干个学生信息，保存到 student 数组中，并将实际读取的学生数量赋值给 n 变量。
    ...
    while(1) {
        ...
        switch(choice) {
            ...
            case 3:  // 按平均成绩排序
                ...
                for(int i = 0; i < n; i++) {
                    float avg = (student[i].score[0] + student[i].score[1] + student[i].score[2]) / 3.0;//求和均分
            ...
            case 4: {  // 使用索引文件快速查找（先用索引文件查到序号，再用Fetch获取学生信息）
                extern int n;
                char key[30];  
                printf("请输入要查找的姓名: ");
                scanf("%s", key);//用户输入要查找的学生姓名，保存在 key 中

                // 加载姓名索引到内存
                NameIndex indexArr[N];
                //indexArr 是一个索引数组，读取进内存后，每一项都保存了某个学生的姓名和其对应在学生信息文件中的字节位置。
                ...
                int found = 0;//0没找到，1找到了
                int stuIndex = -1;//后面要求为非负
                for (int i = 0; i < n; i++) {
                ...
                if (found && stuIndex >= 0 && stuIndex < N) {
                    STU s = Fetch(stuIndex);//找到了就调用Fetch()读取信息
                    printf("\n找到匹配学生:\n");
                    printf("\n%-16s %-14s %-12s %-7s %-9s %-9s %-8s\n", 
                          "学号", "姓名", "专业", "班级", "数学A", "物理B", "化学C");
                    Output(&s);  
            ...
            case 5: {
                ...
                Search(student, classNo, s, scoreSum);//调用search()进行综合查找
                break;
            }
            ...
            case 6: {
                ...
                Sort_buble(student, classNo);//调用Sort_buble()进行排序
                break;
            }
            ...
            case 7: {
                ...
                ModifyTopStudent(student, classNo);//调用ModifyTopStudent()进行查询与修改
                break;
            }
 ...
 int Input2(STU *p, const char *filename) {//因与要求中的函数名不一致，后面被封装进Input
...
void CreateNameIndex(const char *filename) {//创建姓名索引文件
...
void Sort_insert2(STU *p, int n, char *major, int courseIndex) {//后面被封装入Sort_insert()
    // 局部变量
...
void AddStudent(STU *p, int *n) {//增加学生信息
    if (*n >= N) {
        printf("学生数量已满，无法添加！\n");
        return;
    }
...
void DeleteStudent(STU *p, int *n, char *delName) {//删除学生信息
    int found = 0;
    for (int i = 0; i < *n; i++) {
...
void ModifyStudentInfo(STU *p, int n, const char *name) {//修改学生信息
    int found = 0;
    for (int i = 0; i < n; i++) {
