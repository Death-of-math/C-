...
int main() {
    ...
    Input(student, N); // N为学生总数
    // 统计实际人数
    n = 0;
    FILE *fp = fopen(currentFileName, "rb");
    if (fp) {
        int n_tmp = 0;
        fread(&n_tmp, sizeof(int), 1, fp); // 跳过人数
        STU tmp;
        while (fread(&tmp, sizeof(STU), 1, fp) == 1 && n < N) {
            n++;
        }
        fclose(fp);
    }
    ...
    while(1) {
        ...
        switch(choice) {
            ...
            case 4: {
                ...
                size_t indexCount = fread(indexArr, sizeof(NameIndex), n, index_fp); // 只读n条
            ...
            int found = 0;
            int stuIndex = -1;
            for (int i = 0; i < indexCount; i++) { // 用indexCount
            ...
            scanf("%s", major);
            // 不要在这里输入课程编号
            Sort_insert(student, n, major); // 只传major
            break;
            }
 ...
 void Input(STU *p, int n) {
    FILE *fp = fopen(currentFileName, "rb"); // 用当前文件名
    if (!fp) {
        printf("无法打开文件 %s\n", currentFileName);
        exit(1);
    }
    int n_tmp = 0;
    fread(&n_tmp, sizeof(int), 1, fp); // 跳过人数
    fread(p, sizeof(STU), n, fp);      // 读取学生数据
    fclose(fp);
}
...
STU Fetch(int studentIndex) {
    ...
    // 跳过人数
    int n_tmp = 0;
    fread(&n_tmp, sizeof(int), 1, fp);

    fseek(fp, studentIndex * sizeof(STU), SEEK_CUR);
...
void CreateNameIndex(const char *filename) {
    ...
    // 跳过人数
    int n_tmp = 0;
    fread(&n_tmp, sizeof(int), 1, data_fp);
    pos = ftell(data_fp);
...
void Sort_insert(STU *p, int n, char *major) {
    ...
    // 1.5 让用户输入课程编号
    printf("请输入要排序的课程编号(0-数学A, 1-物理B, 2-化学C): ");
    scanf("%d", &courseIndex);
    if (courseIndex < 0 || courseIndex > 2) {
        printf("课程编号无效！\n");
        return;
